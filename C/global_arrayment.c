...
struct promoter
{
	char name[BUFSIZE];
	char seq[BUFSIZE];
	char result[BUFSIZE]; //result seq
}pro[GENE_NUMBER];
...
int match_score(int i, int j, int k)
{
	...
	case1 = dp[i-1][j-1][k-1] + get_score(pro[0].seq[i], pro[1].seq[j]) + get_score(pro[0].seq[i], pro[2].seq[k]) + get_score(pro[1].seq[j], pro[2].seq[k]);
...
int traceback(int i, int j, int k)
{
	...
	if(trace[i][j][k] == 0){
		...
		pro[0].result[index] = pro[0].seq[index];
		pro[1].result[index] = pro[1].seq[index];
		pro[2].result[index] = pro[2].seq[index];
	}
...
int main(int argc,char *argv[])
{
	...
	gene_num=read_promoter(argv[1]); /*１番目の引数で指定した転写因子の複数の結合部位配列を読み込む*/
	for(i=0;i<gene_num;i++){
	...
	make_map(seq_len);
	traceback(seq_len-1, seq_len-1, seq_len-1);
