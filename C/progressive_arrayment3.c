...
struct promoter
{
	char name[BUFSIZE];
	char seq[BUFSIZE];
}pro[GENE_NUMBER], result[GENE_NUMBER], cluster[GENE_NUMBER];
/* pro: original data */
...
int traceback(int i, int j, int y)
{
	...
	if(i == 0 && j == 0){
		...
		if(y == 1){/*make result[0] */
			result[0].seq[index] = cluster[0].seq[i-1];
		}
		result[y].seq[index] = cluster[y].seq[j-1];
	}
	else if(trace[i][j] == 2){
		index = traceback(i-1, j-1, y);
		if(y == 1){
...
void make_tree(int count)
{
	...
	strcpy(cluster[i+1].seq, pro[max.y].seq);
}
...
int main(int argc,char *argv[])
{
	...
	if(gene_num % 2 == 1){
		...
		for(i = 0; cluster[i].name[0] != '\0'; i++){
			for(j = 0; j < gene_num; j++){
				if(strcmp(cluster[i].name, pro[j].name)){
					check[j] = 1;
				}
			}
		}
		...
		strcpy(cluster[i].name, pro[j].name);
		strcpy(cluster[j].seq, pro[j].seq);
	}
	for(i = 0; i < gene_num; i++){
		printf("%s\n", cluster[i].name);
	}
	...
	for(i = 1; i < gene_num; i++){
		if(i == 1){
			make_map(seq_len, 0, 1, 0);
			strcpy(result[0].name, cluster[i].name);
		}
		else{
			make_map(seq_len, i, 0, 1);
		}
		strcpy(result[i].name, cluster[i].name);
